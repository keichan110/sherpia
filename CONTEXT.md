...
- **gmail-digest** — 前日ウィンドウ内に届いた `Newsletter` ラベルのメールを、Geminiで1回だけ横断要約（対応が必要／種類別／全体要約）し、親メッセージ＋スレッド返信（件名・送信者の一覧。Block Kit・10件単位）のSlackダイジェストにする。**ダイジェスト型**（設計確定・実装済み。ADR-0002・0003）。
